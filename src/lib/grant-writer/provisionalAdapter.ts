export type RecommendationLevel = 'primary' | 'secondary' | 'optional' | 'ambiguous';
export type ConfidenceBand = 'high' | 'medium' | 'low';
export type ActorRole = 'rights_holder' | 'target_actor' | 'institutional_actor' | 'other';
...
export function adaptProvisionalResponse(raw: unknown): ProvisionalDomainResponse {
  ...
  return {
    ...
    sectors: Array.isArray(r.sectors) ? r.sectors.map(normalizeRecommendation) : [],
    interventions: Array.isArray(r.interventions) ? r.interventions.map(normalizeRecommendation) : [],
    sdgs: Array.isArray(r.sdgs) ? r.sdgs.map(normalizeSdg) : [],
    actorRoles: Array.isArray(r.actorRoles) ? r.actorRoles.map(normalizeActorRole) : [],
    
    ambiguities: Array.isArray(r.ambiguities) ? r.ambiguities.map(normalizeAmbiguity) : [],
    missingInformation: Array.isArray(r.missingInformation) ? r.missingInformation.map(normalizeMissingInfo) : [],
    warnings: Array.isArray(r.warnings) ? r.warnings.map(normalizeWarning) : [],
    blueprint: normalizeBlueprint(r.blueprint)
  };
}

function normalizeRecommendation(item: RawRecommendation): MappingRecommendation {
  return {
    id: String(item.id || ''),
    ...
    evidence: item.evidence ? normalizeEvidence(item.evidence) : undefined
  };
}

function normalizeSdg(item: RawSdg): SDGRecommendation {
  return {
...
function normalizeActorRole(item: RawActorRole): ActorRoleRecommendation {
  return {
    id: String(item.id || ''),
    actorName: String(item.actorName || ''),
    role: (item.role || 'other') as ActorRole,
    level: (item.level || 'optional') as RecommendationLevel,
    confidence: (item.confidence || 'low') as ConfidenceBand,
    explanation: String(item.explanation || ''),
    evidence: item.evidence ? normalizeEvidence(item.evidence) : undefined
...
function normalizeEvidence(ev: RawEvidenceSpan): EvidenceSpan {
  return {
    sourceField: String(ev.sourceField || 'programStory'),
    text: String(ev.text || ''),
    startOffset: Number(ev.startOffset || 0),
    endOffset: Number(ev.endOffset || 0)
  };
}

function normalizeAmbiguity(item: RawAmbiguity): AmbiguityItem {
  return {
    id: String(item.id || ''),
...
function normalizeBlueprint(bp: RawBlueprint | undefined): ProgramBlueprint {
