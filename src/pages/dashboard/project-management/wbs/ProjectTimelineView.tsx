...
export default function ProjectTimelineView({
  wbsItems, stages, orgMembers, orgMemberLookup, isOwner, onScheduleChange, onStagesRefresh,
}: Props) {
  ...
  // Activity schedule popover
  const [schedulePopoverId, setSchedulePopoverId] = useState<string | null>(null);
  const [popoverSm, setPopoverSm] = useState('');
  const [popoverDw, setPopoverDw] = useState('');
  ...
  const hasAnyStageDates = useMemo(
    () => stageInputs.some((s) => s.planned_start_date),
    [stageInputs],
  );

  const stagesWithoutDates = activeStages.filter((s) => !(s as any).planned_start_date);
  ...
  const timelineRange = useMemo(() => {
    ...
    for (const s of stageInputs) {
      if (s.planned_start_date) { const d = new Date(s.planned_start_date); if (!minDate || d < minDate) minDate = d; }
      if (s.planned_end_date) { const d = new Date(s.planned_end_date); if (!maxDate || d > maxDate) maxDate = d; }
    }
    ...
    while (cursor <= maxDate) {
      const mLabel = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'][cursor.getMonth()];
      months.push({ year: cursor.getFullYear(), month: cursor.getMonth(), label: `${mLabel} ${cursor.getFullYear()}` });
      cursor.setMonth(cursor.getMonth() + 1);
    }
    return { minDate, maxDate, months, totalMonths: months.length };
  }, [hasAnyStageDates, stageInputs, visibleRows]);
  ...
  useLayoutEffect(() => {
    ...
    Object.entries(leftRowRefs.current).forEach(([k, el]) => {
      if (el) { const h = el.offsetHeight; if (h && rowHeights[k] !== h) { newH[k] = h; changed = true; } }
    });
    if (changed) setRowHeights((prev) => ({ ...prev, ...newH }));
  });

  // Stage schedule edit
  const openStageEdit = useCallback((s: Stage) => {
    setEditingStage(s);
    setStageStart((s as any).planned_start_date || '');
    setStageFinish((s as any).planned_end_date || '');
    setStageDialogOpen(true);
  }, []);

  const saveStageSchedule = useCallback(async () => {
    if (!editingStage || !stageStart || !stageFinish) {
    ...
    try {
      ...
      toast({ title: 'Gagal menyimpan jadwal', description: err?.message, variant: 'destructive' });
    } finally { setSavingStage(false); }
  }, [editingStage, stageStart, stageFinish, toast, onStagesRefresh]);

  // Activity/Task schedule popover
  const openSchedulePopover = useCallback((row: VisibleRow) => {
    if (!row.wbsId) return;
  ...
  const saveActivitySchedule = useCallback((wbsId: string) => {
    const sm = parseInt(popoverSm);
    const dw = parseInt(popoverDw);
    if (sm < 1 || dw < 1) return;
    if (onScheduleChange) onScheduleChange(wbsId, 'start_month', sm);
    if (onScheduleChange) onScheduleChange(wbsId, 'duration_weeks', dw);
    setSchedulePopoverId(null);
  }, [popoverSm, popoverDw, onScheduleChange]);

  const stagesMissing = stagesWithoutDates.length;
            ...
            {stagesWithoutDates.map((s) => (
              <div key={s.id} className="flex items-center justify-between p-3 border rounded-lg bg-slate-50 dark:bg-slate-800/50">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {isOwner && (
                    <Button size="sm" variant="outline" className="h-7 text-[10px]" onClick={() => openStageEdit(s)}>
  ...
  return (
    <div className="border rounded-lg bg-white dark:bg-slate-900 flex flex-col" style={{ height: 'calc(100vh - 280px)', minHeight: 400 }}>
      {/* Stage schedule warning */}
      {stagesMissing > 0 && (
        <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-950/20 border-b text-xs">
          <AlertTriangle className="h-3.5 w-3.5 text-amber-600" />
          <span className="text-amber-800 dark:text-amber-300">{stagesMissing} Stage belum memiliki jadwal. Atur jadwal agar Activity muncul di Gantt.</span>
        </div>
      )}

      {/* Sticky headers */}
      <div className="flex border-b bg-slate-50 dark:bg-slate-900 shrink-0">
        <div className="py-2 px-4 text-[10px] font-bold uppercase text-slate-500 border-r" style={{ width: LEFT_WIDTH }}>
          Timeline
        </div>
                    ...
                    <div className="flex items-center justify-between w-full">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        {row.schedule.missingSchedule ? (
                          isOwner && <Button size="sm" variant="outline" className="h-5 text-[8px] px-1.5" onClick={() => {
                            const s = stages.find((st) => st.id === row.stageId);
                            if (s) openStageEdit(s);
                          }}><Settings className="h-2.5 w-2.5 mr-0.5" /> Atur Jadwal</Button>
                        ) : (
                          <>
                            <span className="text-[9px] text-muted-foreground font-normal">
                              {row.schedule.derivedStartDate ? formatDate(row.schedule.derivedStartDate) : '—'}
                            </span>
                            <span className="text-[9px] text-muted-foreground">→</span>
                            <span className="text-[9px] text-muted-foreground font-normal">
                              {row.schedule.derivedFinishDate ? formatDate(row.schedule.derivedFinishDate) : '—'}
                            </span>
                            {isOwner && <Button size="sm" variant="ghost" className="h-5 text-[8px] px-1" onClick={() => {
                              const s = stages.find((st) => st.id === row.stageId);
                              if (s) openStageEdit(s);
                            }}>Edit</Button>}
                          </>
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        {row.isMissingSchedule ? (
                          <span className="text-[8px] text-amber-600 flex items-center gap-0.5">
                            <Calendar className="h-2.5 w-2.5" /> Tenggat Belum Diatur
                          </span>
                        ) : (
                        ...
                        {isOwner && row.wbsId && (
                          <Popover open={schedulePopoverId === row.wbsId} onOpenChange={(o) => { if (!o) setSchedulePopoverId(null); }}>
                            <PopoverTrigger asChild>
                              <Button size="sm" variant="outline" className="h-5 text-[8px] px-1.5 ml-auto shrink-0"
                                onClick={() => openSchedulePopover(row)}>
                            ...
                            <PopoverContent className="w-60 text-xs" align="start">
                              <div className="space-y-2">
                                <div>
                                  <Label className="text-[9px]">Mulai Bulan</Label>
                                  <Input type="number" min={1} max={36} value={popoverSm}
                                    onChange={(e) => setPopoverSm(e.target.value)} className="h-7 text-xs" />
                                </div>
                                <div>
                                  <Label className="text-[9px]">Durasi Minggu</Label>
                                  <Input type="number" min={1} max={52} value={popoverDw}
                                    onChange={(e) => setPopoverDw(e.target.value)} className="h-7 text-xs" />
                                </div>
                                {(() => {
                                  const sm = parseInt(popoverSm); const dw = parseInt(popoverDw);
                                  if (sm >= 1 && dw >= 1) {
                                    const stage = stages.find((s) => s.id === (row.stageId)) as any;
                                    const anchor = stage?.planned_start_date;
                                    if (anchor) {
                                      const start = new Date(anchor); start.setMonth(start.getMonth() + sm - 1);
                                      const finish = new Date(start.getTime() + dw * 7 * 86400000);
                                      return (
                                        <div className="text-[9px] text-muted-foreground space-y-0.5">
                                          <div>Perkiraan Mulai: {formatDate(start)}</div>
                                          <div>Perkiraan Selesai: {formatDate(finish)}</div>
                                        </div>
                                      );
                                    }
                                  }
                                ...
                                <Button size="sm" className="w-full text-xs h-7" onClick={() => saveActivitySchedule(row.wbsId!)}>
                                  Simpan
                                </Button>
                              </div>
                            </PopoverContent>
                ...
                return (
                  <div key={`g-${row.key}`} className="border-b border-slate-100 dark:border-slate-800 relative flex items-center" style={{ height: h }}>
                    ...
                    {row.isMissingSchedule && (row.type === 'activity' || row.type === 'task') && (
                      <div className="absolute top-2 left-2 text-[8px] text-amber-600 italic bg-amber-50/80 px-1.5 py-0.5 rounded border border-amber-200">
                        Belum diatur
                      </div>
                    )}
                  </div>
      ...
      <Dialog open={stageDialogOpen} onOpenChange={setStageDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm">Atur Jadwal Stage</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              <Label className="text-xs">Stage</Label>
              <Input value={editingStage?.title || ''} disabled className="h-8 text-xs bg-slate-50" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-xs">Tanggal Mulai</Label>
                <Input type="date" value={stageStart} onChange={(e) => setStageStart(e.target.value)} className="h-8 text-xs" />
              </div>
              <div>
                <Label className="text-xs">Tanggal Selesai</Label>
                <Input type="date" value={stageFinish} onChange={(e) => setStageFinish(e.target.value)} className="h-8 text-xs" />
              </div>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button size="sm" className="text-xs" onClick={saveStageSchedule} disabled={savingStage}>
              {savingStage ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : null} Simpan
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
