...
interface MEALPlannerProps {
  ...
  sector?: string;
  /**
   * 'programme_design' (default) keeps LFA auto-import as the primary flow,
   * unchanged. 'project_management' skips the auto-import round trip (there
   * are zero lfa_entries by design there) and shows a per-indicator
   * Objective/Stage/Deliverable context picker.
   */
...
export default function MEALPlanner({
  ...
  const { user } = useAuth();
  const [objectiveOptions, setObjectiveOptions] = useState<MealContextOption[]>([]);
  const [stageOptions, setStageOptions] = useState<MealContextOption[]>([]);
  const [deliverableOptions, setDeliverableOptions] = useState<MealContextOption[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Check if we need to do first-time auto-import from LFA -- Programme
      // Design only; a Project Management project has zero lfa_entries by
      // design, so this would be a pointless round-trip (and a confusing
      // "LFA Matrix kamu kosong" toast) there.
      if ((items || []).length === 0 && productMode !== 'project_management') {
  ...
  }, [projectId, toast, productMode]);

  // Project Management only: Objectives/Stages/Deliverables for this
  // project, used to populate the per-indicator context picker.
  const loadContextOptions = useCallback(async () => {
    try {
      // project_objectives/project_stages/programme_deliverables are not yet
      // reflected in the generated Database type (types are
      // Production-canonical and are not regenerated by this change), so
      // these queries are narrowly cast.
      const [objRes, stageRes, delivRes] = await Promise.all([
        (supabase as any).from('project_objectives').select('id, title').eq('project_id', projectId),
        (supabase as any).from('project_stages').select('id, title').eq('project_id', projectId),
        (supabase as any).from('programme_deliverables').select('id, title').eq('lfa_project_id', projectId),
      ]);
      setObjectiveOptions((objRes.data || []) as MealContextOption[]);
      setStageOptions((stageRes.data || []) as MealContextOption[]);
      setDeliverableOptions((delivRes.data || []) as MealContextOption[]);
    } catch (err) {
      console.error('[Impactory] Error loading MEAL context options:', err);
    }
  }, [projectId]);

  // Project Management only: assign/change/unlink a MEAL item's Objective/
  // Stage/Deliverable context via the RPC (never a direct table write for
  // these fields).
  const handleAssignMealContext = async (
    mealItemId: string,
    next: { objectiveId?: string | null; stageId?: string | null; deliverableId?: string | null },
  ) => {
    const current = mealItems.find((m) => m.id === mealItemId);
    if (!current) return;
    const objectiveId = 'objectiveId' in next ? next.objectiveId ?? null : current.objective_id ?? null;
    const stageId = 'stageId' in next ? next.stageId ?? null : current.stage_id ?? null;
    const deliverableId = 'deliverableId' in next ? next.deliverableId ?? null : current.deliverable_id ?? null;
    try {
      const { error: rpcError } = await (supabase.rpc as any)('assign_meal_item_context', {
      ...
      setMealItems((prev) =>
        prev.map((m) =>
          m.id === mealItemId
            ? { ...m, objective_id: objectiveId, stage_id: stageId, deliverable_id: deliverableId }
            : m,
        ),
      );
    } catch (err: any) {
      toast({
        title: 'Gagal mengubah context',
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-700 dark:text-slate-300 sticky top-0 z-10 shadow-xs">
                ...
                {productMode === 'project_management' && <th className="p-3.5 py-4 w-40 shrink-0">Context</th>}
              ...
              {mealItems.map((item, index) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 group transition-all align-top">
                    ...
                    {/* Context (Project Management only) */}
                    {productMode === 'project_management' && (
                      <td className="p-3.5">
                        <div className="flex flex-col gap-1">
                          <select
                            value={item.objective_id || ''}
                            onChange={(e) => void handleAssignMealContext(item.id, { objectiveId: e.target.value || null })}
                            title="Objective"
                            data-testid="meal-context-objective-picker"
                          ...
                          <select
                            value={item.stage_id || ''}
                            onChange={(e) => void handleAssignMealContext(item.id, { stageId: e.target.value || null })}
                            title="Stage"
                            data-testid="meal-context-stage-picker"
                            className="text-[10px] w-full border bg-transparent rounded px-1 h-6 focus:outline-none dark:border-slate-800"
                          >
                          ...
                          <select
                            ...
                            onChange={(e) => void handleAssignMealContext(item.id, { deliverableId: e.target.value || null })}
                            title="Deliverable"
                            data-testid="meal-context-deliverable-picker"
                            className="text-[10px] w-full border bg-transparent rounded px-1 h-6 focus:outline-none dark:border-slate-800"
                          >
