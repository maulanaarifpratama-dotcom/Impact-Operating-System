...
interface WBSBuilderProps {
   ...
   * unchanged. 'project_management' skips auto-import (there are zero
   * lfa_entries to import from), shows a manual-creation empty state instead
   * of "Selesaikan LFA Matrix...", and enables manual Level 1 creation plus
   * per-Level-1 Stage assignment.
   */
  productMode?: 'programme_design' | 'project_management';
}

interface WbsStageOption {
  id: string;
  title: string;
...
export default function WBSBuilder({
  ...
  const loadWbsItems = async () => {
    ...
    try {
      ...
      if ((!data || data.length === 0) && productMode !== 'project_management') {
        // Trigger Auto-import from LFA -- Programme Design only; a Project
        // Management project has zero lfa_entries by design, so this would
        // be a pointless round-trip there.
        await performAutoImport();
      } else {
        const rows = (data || []) as WbsItem[];
        setWbsItems(rows);
        // Sync global mode from first item if exists
        if (rows[0]?.mode) {
          setGlobalMode(rows[0].mode as 'simple' | 'professional');
        }
      }
  ...
  // Project Management only: Stages for this project, used to populate the
  // per-Level-1 Stage picker. Not fetched at all in Programme Design mode.
  const loadStages = async () => {
    try {
      // project_stages is not yet reflected in the generated Database type
      ...
      setStages((data || []) as WbsStageOption[]);
    } catch (err: any) {
      console.error('[Impactory] Error loading stages:', err);
    }
  };

  // Project Management only: assign/move/unassign a WBS item's Stage via the
  // RPC (never a direct table write for this field).
  const handleAssignStage = async (wbsItemId: string, stageId: string | null) => {
    try {
      const { error: rpcError } = await (supabase.rpc as any)('assign_wbs_item_to_stage', {
        p_wbs_item_id: wbsItemId,
        p_stage_id: stageId,
      });
      if (rpcError) throw rpcError;

      setWbsItems((prev) => prev.map((i) => (i.id === wbsItemId ? { ...i, stage_id: stageId } : i)));
    } catch (err: any) {
      toast({
        title: 'Gagal mengubah Stage',
  ...
  const performAutoImport = async () => {
    ...
    try {
      ...
      if (newWbsItems.length > 0) {
        // WbsItem now carries an optional stage_id (not yet in the generated
        // Database type), which breaks strict structural matching against
        // the insert overload even though this particular insert never sets
        // it -- narrowly cast, matching the same reason used elsewhere.
        const { error: insErr } = await supabase
          .from('lfa_wbs_items')
          .insert(newWbsItems as any);
  ...
  // Project Management only: manual Level 1 (root, Output-equivalent)
  // creation. Neither handleAddSubActivity nor handleAddTask supports a
  // parentless item (both require an existing parentItem), so this is new.
  const handleAddRootItem = async () => {
    ...
    try {
      ...
      const nextSortOrder = level1Items.length > 0
        ? Math.max(...level1Items.map((i) => i.sort_order)) + 1
        : 0;
              ...
              return (
                <Fragment key={item.id}>
                  ...
                  >
                    ...
                    {/* Stage Picker (Project Management only, Level 1 items) */}
                    {item.level === 1 && productMode === 'project_management' && (
                      <select
                        value={item.stage_id || ''}
                        onChange={(e) => void handleAssignStage(item.id, e.target.value || null)}
                        title="Stage"
                        data-testid="wbs-stage-picker"
                    ...
                    <div className="w-8 flex items-center justify-end gap-1 shrink-0">
                      {item.level === 1 && (
                        <Button
                          ...
                          disabled={productMode !== 'project_management'} // Read-only from LFA Matrix in Programme Design
                        >
